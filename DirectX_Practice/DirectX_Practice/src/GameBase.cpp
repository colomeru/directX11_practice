...
bool GameBase::Run(HINSTANCE hIns)
{
	...
	while (Window::GetInstance()->MessageHandling())
	{
		...
		if (debugDraw)
		{
			FontManager::GetInstance()->Draw(Vector2(10.0f,  30.0f), MyUtil::toString("ライト回転     : A/D"));
			FontManager::GetInstance()->Draw(Vector2(10.0f,  50.0f), MyUtil::toString("モデル回転     : Z/X"));
			FontManager::GetInstance()->Draw(Vector2(10.0f,  70.0f), MyUtil::toString("モデル切り替え   : C"));
			FontManager::GetInstance()->Draw(Vector2(10.0f,  90.0f), MyUtil::toString("カメラ移動     : 十字キー"));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 110.0f), MyUtil::toString("シェーダー切り替え : 1/2/3/4"));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 130.0f), MyUtil::toString("影解像度切り替え 　: Q/E"));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 200.0f), MyUtil::toString("輝度の閾値   : Y/G : %.2f", brightPassThreshold));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 220.0f), MyUtil::toString("元画像の彩度  : P/L : %.2f", baseSaturation));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 240.0f), MyUtil::toString("元画像の輝度  : O/K : %.2f", baseIntensity));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 260.0f), MyUtil::toString("ブルームの彩度 : I/J : %.2f", bloomSaturation));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 280.0f), MyUtil::toString("ブルームの輝度 : U/H : %.2f", bloomIntensity));
			FontManager::GetInstance()->Draw(Vector2(10.0f, 300.0f), MyUtil::toString("パラメータの初期化 : R"));
		}
		else
		{
