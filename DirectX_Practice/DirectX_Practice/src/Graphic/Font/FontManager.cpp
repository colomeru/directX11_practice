...
void FontManager::Create(const std::string & fileName, int fontSize)
{
	// フォント保存用アトラステクスチャの生成
	UINT size = 256;
...
void FontManager::Draw(Vector2 position, const std::string & text)
{
	...
	Effect effect(VERTEX_SHADER_ID::FONT_SHADER, PIXEL_SHADER_ID::TEXTURE_SHADER);
...
void FontManager::CreateCharacter(TCHAR * c)
{
	m_FontTexture.Begin();
	...
	// 文字データ生成
	character.pTexture->Create(font.Create(m_FontName, c, 256));
...
void FontManager::CreateCharacter(const std::string & str)
{
